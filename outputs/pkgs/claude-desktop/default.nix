...
# to bump: update repoSrc rev/hash + version + the windows exe srcs below
{
  ...
  srcs = {
    ...
    aarch64-linux = fetchurl {
      url = "https://downloads.claude.ai/releases/win32/arm64/1.12603.1/Claude-3df4fd263723119bc45f0af2d784afd5055e2ba9.exe";
  ...
  stdenvNoCC.mkDerivation {
    ...
    buildPhase = ''
      ...
      substituteInPlace ./upstream/scripts/patches/config.sh \
        ...
        --replace-fail 'code = code.replace(match[0], filtered);' 'code = code.split(match[0]).join(filtered);'
